...
class Checkout extends Main_Controller {
	...
	public function index() {
		...
		if ( ! $this->customer->islogged() AND $this->config->item('guest_order') !== '1') { 											// else if customer is not logged in
			$this->alert->set('alert', $this->lang->line('alert_not_logged'));
  			redirect('login');															// redirect to account register page and display error
		}
		...
		$data['text_login_register']	= $this->customer->isLogged() ? sprintf($this->lang->line('text_logout'), $this->customer->getFirstName(), site_url('logout')) : sprintf($this->lang->line('text_registered'), site_url('login'));
		...
		if ($this->input->post('post_checkout')) {
			...
			$data['button_back'] 		= '<a class="btn btn-default" onClick="$(\'#checkout\').fadeIn();$(\'#payment\').empty();$(this).fadeOut();$(\'.btn-checkout\').html(\'Payment\')">'. $this->lang->line('button_back') .'</a>';
	...
	public function paypal() {
		if ($this->input->get('token') AND $this->input->get('PayerID')) { 						// check if token and PayerID is in $_GET data
			...
			$order_id = (is_numeric($this->session->tempdata('last_order_id'))) ? $this->session->tempdata('last_order_id') : FALSE;
	...
	public function success() {
		...
		if (is_numeric($this->session->tempdata('last_order_id'))) {
			$order_info = $this->Orders_model->getOrder($this->session->tempdata('last_order_id'), $customer_id);	// retrieve order details array from getMainOrder method in Orders model
		} else {
			$this->session->unset_tempdata('last_order_id');
			$order_info = array();
		}

		if (!$order_info OR empty($order_info['status_id'])) {																	// checks if array is returned
			$this->alert->set('alert', $this->lang->line('alert_server_error'));
			redirect('checkout');
		} else {																				// else redirect to checkout
			$data['message'] = sprintf($this->lang->line('text_success_message'), $order_info['order_id'], site_url('orders'));
			...
			if (isset($payment['title'])) { 										// checks if payment method is paypal or cod
		...
		if ($this->validateForm() === TRUE) {
			...
			if ($this->input->post('order_type') === '1') {
				if ($this->input->post('new_address') === '1') {								// checks if new-address $_POST data is set else use existing address $_POST data
					$order_data['address_id'] = $this->Addresses_model->addAddress($order_data['customer_id'], $this->input->post('address')); 	// send new-address $_POST data and customer id to addAddress method in Customers model
	...
	public function _validatePayment() {
		...
		if (is_numeric($this->input->post('post_checkout')) AND $this->input->post('payment')) {
			$order_data['payment'] = $this->input->post('payment');

			$order_id = (is_numeric($this->session->tempdata('last_order_id'))) ? $this->session->tempdata('last_order_id') : FALSE;
			$order_info = $this->Orders_model->getOrder($order_id, $order_data['customer_id']);	// retrieve order details array from getMainOrder method in Orders model

			if ($order_info AND empty($order_info['status_id'])) {
				$order_data['last_order_id'] = $order_id;
			}

			$order_id = $this->Orders_model->addOrder($order_data, $cart_contents);
			$this->session->set_tempdata('last_order_id', $order_id);
	...
	public function validate_address() {
		...
		if (!empty($address['postcode'])) {
			...
			$address = $this->Addresses_model->getAddress($this->customer->getId(), $this->input->post('existing_address')); 	// send new-address $_POST data and customer id to addAddress method in Customers model
			if (!$address) {
