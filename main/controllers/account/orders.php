...
class Orders extends Main_Controller {
	...
	public function index() {
		...
		foreach ($results as $result) {
			...
			$data['orders'][] = array(															// create array of customer orders to pass to view
				...
				'reorder' 				=> site_url('account/orders/reorder/'. $result['order_id']),
	...
	public function view() {
		...
		$data['reorder_url'] 			= site_url('account/orders/reorder/'. $order_id);
		$data['button_back'] 			= $this->lang->line('button_back');
		...
		if ($result) {
            ...
            $menu_options = $this->Orders_model->getOrderMenuOptions($result['order_id']);
			foreach ($order_menus as $order_menu) {
				$option_data = array();

				if (!empty($menu_options[$order_menu['menu_id']])) {
		 			foreach ($menu_options[$order_menu['menu_id']] as $menu_option) {
