...
class Reviews extends Main_Controller {
	...
	public function index() {
		...
		$this->template->setBreadcrumb($this->lang->line('text_heading'), 'reviews');
		...
		foreach ($results as $result) {
			$data['reviews'][] = array(															// create array of customer reviews to pass to view
				...
				'view' 				=> site_url('reviews/view/'. $result['review_id'])
			);
		}

		$prefs['base_url'] 			= site_url('reviews').$url;
	...
	public function view() {
		...
		$result = $this->Reviews_model->getReview($this->customer->getId(), $review_id);								// retrieve specific customer message based on message id to be passed to view
		...
		$this->template->setBreadcrumb('<i class="fa fa-home"></i>', '/');
		$this->template->setBreadcrumb($this->lang->line('text_heading'), 'reviews');
		$this->template->setBreadcrumb($this->lang->line('text_view_review'), 'reviews/view');
		...
		$this->template->setPartials(array('header', 'content_top', 'content_left', 'content_right', 'content_bottom', 'footer'));
		$this->template->render('review_view', $data);
	}
	...
	public function add() {
		...
		$data['action']	= site_url('reviews/add/'. $sale_type .'/'. $sale_id .'/'. $location_id);
		...
		$this->template->setBreadcrumb('<i class="fa fa-home"></i>', '/');
		$this->template->setBreadcrumb($this->lang->line('text_heading'), 'reviews');
		$this->template->setBreadcrumb($this->lang->line('text_write_review'), 'reviews/add');
		...
		$this->template->setPartials(array('header', 'content_top', 'content_left', 'content_right', 'content_bottom', 'footer'));
		$this->template->render('review_add', $data);
	}
