...
class Reserve_table extends Main_Controller {
	...
	public function index() {
		...
		$this->template->setBreadcrumb($this->lang->line('text_heading'), 'reserve_table');
		...
		$data['text_login_register']		= $this->customer->isLogged() ? sprintf($this->lang->line('text_logout'), site_url('logout')) : sprintf($this->lang->line('text_login'), site_url('login'));
		...
		$data['action'] 					= site_url('reserve_table');
		$data['continue'] 					= site_url('reserve_table/success');
		...
		if (!empty($this->session->userdata('reservation')) AND $this->input->get('reserve_time')) {
			$data['show_reserve'] = TRUE;
		}

		if ($this->input->post() AND $this->_reserveTable() === TRUE) {
			redirect('reserve_table/success');
		}

		$this->template->setPartials(array('header', 'content_top', 'content_left', 'content_right', 'content_bottom', 'footer'));
		$this->template->render('reserve_table', $data);
	}

	public function success() {
		$this->template->setBreadcrumb('<i class="fa fa-home"></i>', '/');
		$this->template->setBreadcrumb($this->lang->line('text_heading'), 'reserve_table');
		$this->template->setBreadcrumb($this->lang->line('text_heading_success'), 'reserve_table/success');
...
/* Location: ./main/controllers//reserve_table.php */