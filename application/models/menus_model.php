...
class Menus_model extends CI_Model {
	...
	public function getAdminOption($option_id) {		
		$this->db->from('options');
		$this->db->where('option_id', $option_id);
	...
	public function getAdminMenuOptions($menu_id = FALSE) {		
		$results = array();
			...
			if ($query->num_rows() > 0) {
				foreach ($query->result_array() as $row) {
					$results[] = array(
						...
						'option_values'		=> $this->getAdminMenuOptionValues($row['menu_option_id'], $row['option_id'])
					);
				}				
			}
		}
	...
	public function getAdminMenuOptionValues($menu_option_id = FALSE, $option_id = FALSE) {		
		$result = array();
		...
		if ($query->num_rows() > 0) {
			foreach ($query->result_array() as $row) {
				$results[] = array(
					...
					'display_type'		=> $row['display_type'],
					'priority'			=> $row['priority'],
					'option_values'		=> $this->getMainMenuOptionValues($row['menu_option_id'], $row['option_id'])
				);
			}				
		}
	...
	public function getMainMenuOptionValues($menu_option_id = FALSE, $option_id = FALSE) {		
		$result = array();
