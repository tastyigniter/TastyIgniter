...
class Local extends MX_Controller {
	...
	public function index() {
		...
		$data['last_order_time'] 	= sprintf($this->lang->line('text_last_order_time'), $this->location->lastOrderTime());
		$data['payments'] 			= sprintf($this->lang->line('text_payments'), $this->location->paymentsList());
	...
	public function locations() {
		...
		if ($locations) {
			foreach ($locations as $location) {															// loop through menus array
				...
				$total_reviews = sprintf($this->lang->line('text_total_review'), site_url('main/local/reviews'), 1);
				...
				$data['locations'][] = array( 															// create array of menu data to be sent to view
					...
					'offers'				=> $offers,
					'min_total' 			=> (isset($location['min_total']) AND $location['min_total'] > 0) ? $this->currency->format($location['min_total']) : $this->currency->format('0.00'),			
					...
					'delivery_time' 		=> sprintf($this->lang->line('text_delivery_time'), $location['delivery_time']),			
					'collection_time' 		=> sprintf($this->lang->line('text_collection_time'), $location['collection_time']),			
					'href'					=> site_url('main/local?location_id='. $location['location_id'])
				);
			}	
		}
	...
	public function reviews() {
		...
		$data['text_total_review'] = sprintf($this->lang->line('text_total_review'), site_url('main/local/reviews'), $total_reviews);
		...
		$this->template->regions(array('header', 'content_top', 'content_left', 'content_right', 'footer'));
		if (file_exists(APPPATH .'views/themes/main/'.$this->config->item('main_theme').'local_reviews.php')) {
			$this->template->render('themes/main/'.$this->config->item('main_theme'), 'local_reviews', $data);
		} else {
			$this->template->render('themes/main/default/', 'local_reviews', $data);
		}
	}
}
