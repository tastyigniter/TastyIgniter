...
class Banners extends CI_Controller {
	...
	public function index() {
		...
		$this->template->setButton('Modules', array('class' => 'btn btn-default pull-right', 'href' => site_url(ADMIN_URI.'/extensions')));
		...
		foreach ($results as $result) {					
			$data['banners'][] = array(
				...
				'edit' 			=> site_url(ADMIN_URI.'/banners/edit?id=' . $result['banner_id'])
			);
		}
		
		if ($this->input->post('delete') AND $this->_deleteBanner() === TRUE) {
			redirect(ADMIN_URI.'/banners');  			
		}	

		$this->template->regions(array('header', 'footer'));
		if (file_exists(APPPATH .'views/themes/'.ADMIN_URI.'/'.$this->config->item('admin_theme').'banners.php')) {
			$this->template->render('themes/'.ADMIN_URI.'/'.$this->config->item('admin_theme'), 'banners', $data);
		} else {
			$this->template->render('themes/'.ADMIN_URI.'/default/', 'banners', $data);
		}
	}
	...
	public function edit() {
		...
		if ($banner_info) {
			...
			$data['action']	= site_url(ADMIN_URI.'/banners/edit?id='. $banner_id);
		} else {
		    $banner_id = 0;
			$data['action']	= site_url(ADMIN_URI.'/banners/edit');
		}
		...
		$this->template->setBackButton('btn-back', site_url(ADMIN_URI.'/banners'));
		...
		if ($this->input->post() AND $this->_addBanner() === TRUE) {
			if ($this->input->post('save_close') !== '1' AND is_numeric($this->input->post('insert_id'))) {	
				redirect(ADMIN_URI.'/banners/edit?id='. $this->input->post('insert_id'));
			} else {
				redirect(ADMIN_URI.'/banners');
			}
		}
		...
		if ($this->input->post() AND $this->_updateBanner() === TRUE) {
			...
			redirect(ADMIN_URI.'/banners/edit?id='. $banner_id);
		}
		
		$this->template->regions(array('header', 'footer'));
		if (file_exists(APPPATH .'views/themes/'.ADMIN_URI.'/'.$this->config->item('admin_theme').'banners_edit.php')) {
			$this->template->render('themes/'.ADMIN_URI.'/'.$this->config->item('admin_theme'), 'banners_edit', $data);
		} else {
			$this->template->render('themes/'.ADMIN_URI.'/default/', 'banners_edit', $data);
		}
	}
	...
	public function _addBanner() {
    	if ( ! $this->user->hasPermissions('modify', ADMIN_URI.'/banners')) {
			...
			if ($_POST['insert_id'] = $this->Design_model->addBanner($add)) {
				$this->session->set_flashdata('alert', '<p class="alert-success">Banner added sucessfully.</p>');
			} else {
	...
	public function _updateBanner() {
    	if ( ! $this->user->hasPermissions('modify', ADMIN_URI.'/banners')) {
			...
			if ($this->Design_model->updateBanner($update)) {
				$this->session->set_flashdata('alert', '<p class="alert-success">Banner updated sucessfully.</p>');
			} else {
				$this->session->set_flashdata('alert', '<p class="alert-warning">An error occured, nothing added.</p>');				
			}
	...
	public function _deleteBanner() {
    	if ( ! $this->user->hasPermissions('modify', ADMIN_URI.'/banners')) {
			...
			$this->session->set_flashdata('alert', '<p class="alert-success">Banner deleted sucessfully!</p>');
		}
