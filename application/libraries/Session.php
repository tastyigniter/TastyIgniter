...
class  CI_Session {
    ...
    */
    function _session_id_expired()
    {
        ...
        if ( ( $_SESSION['last_activity'] <=  $expiry_time ) OR !isset( $_SESSION['token'] ))
        {
            return true;
        }
    ...
    /**
    * Create secure regenerated_id
    */
    function _create_token() {
		// generate random regenerated id
		$token = '';
		...
		// To make the session ID even more secure we'll combine it with the user's IP
		$token .= $this->CI->input->ip_address();

		// Turn it into a hash
		$token_hash = md5(uniqid($token, TRUE));
