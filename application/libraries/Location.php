...
class Location {
	...
	public function getAddress($format = FALSE) {
	...
	public function openingHours($location_id = '', $weekday = '') {
		if (is_numeric($location_id) AND !empty($weekday) AND isset($this->opening_hours[$location_id])) {
			...
			return (isset($this->opening_hours[$this->location_id])) ? $this->opening_hours[$this->location_id] : array();
		}
	}
				...
				if (!empty($options['delivery_areas']) AND is_array($options['delivery_areas'])) {
					foreach ($options['delivery_areas'] as $key => $area) {
						...
						if (isset($circle[0]->center) AND isset($circle[1]->radius)) {
							$polygons[$result['location_id']][$key]['circle'] = array('center' => $circle[0]->center->k.'|'.$circle[0]->center->A, 'radius' => $circle[1]->radius);
						}
