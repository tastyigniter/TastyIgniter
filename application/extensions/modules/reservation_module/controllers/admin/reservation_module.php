...
class Reservation_module extends MX_Controller {
	...
	public function index() {
    	...
    	if (!$this->user->hasPermissions('access', ADMIN_URI.'/reservation_module')) {
		...
		$extension = $this->Extensions_model->getExtension('module', 'reservation_module');
		
		if (!$this->input->get('id') AND !$this->input->get('name') AND $this->input->get('action') !== 'edit') {
			redirect(ADMIN_URI.'/extensions/edit?name=reservation_module&action=edit&id='.$extension['extension_id']);
		}
		...
		if (file_exists(EXTPATH .'modules/reservation_module/views/admin/reservation_module.php')) { 								//check if file exists in views folder
			$this->load->view('reservation_module/admin/reservation_module', $data);
		} else {
	...
	public function _updateModule() {
    	if (!$this->user->hasPermissions('modify', ADMIN_URI.'/reservation_module')) {
			...
			if ($this->Extensions_model->updateExtension($update, '1')) {
				$this->session->set_flashdata('alert', '<p class="alert-success">Reservation Module updated sucessfully.</p>');
			} else {
...
/* Location: ./application/extensions/modules/reservation_module/controllers/admin/reservation_module.php */