...
class Account_module extends MX_Controller {
	...
	public function index() {
    	...
    	if (!$this->user->hasPermissions('access', ADMIN_URI.'/account_module')) {
		...
		if (file_exists(EXTPATH .'modules/account_module/views/admin/account_module.php')) { 								//check if file exists in views folder
			$this->load->view('account_module/admin/account_module', $data);
		} else {
	...
	public function _updateModule() {
    	if (!$this->user->hasPermissions('modify', ADMIN_URI.'/account_module')) {
			...
			if ($this->Extensions_model->updateExtension($update, '1')) {
				$this->session->set_flashdata('alert', '<p class="alert-success">Account Module updated sucessfully.</p>');
			} else {
...
/* Location: ./application/extensions/modules/account_module/controllers/admin/account_module.php */