...
class Local_module extends MX_Controller {
	...
	public function index() {
    	...
    	if (!$this->user->hasPermissions('access', ADMIN_URI.'/local_module')) {
		...
		$extension = $this->Extensions_model->getExtension('module', 'local_module');
		
		if (!$this->input->get('id') AND !$this->input->get('name') AND $this->input->get('action') !== 'edit') {
			redirect(ADMIN_URI.'/extensions/edit?name=local_module&action=edit&id='.$extension['extension_id']);
		}
		...
		if (file_exists(EXTPATH .'modules/local_module/views/admin/local_module.php')) { 								//check if file exists in views folder
			$this->load->view('local_module/admin/local_module', $data);
		} else {
	...
	public function _updateModule() {
    	if (!$this->user->hasPermissions('modify', ADMIN_URI.'/local_module')) {
			...
			if ($this->Extensions_model->updateExtension($update, '1')) {
				$this->session->set_flashdata('alert', '<p class="alert-success">Local Module updated sucessfully.</p>');
			} else {
...
/* Location: ./application/extensions/modules/local_module/controllers/admin/local_module.php */