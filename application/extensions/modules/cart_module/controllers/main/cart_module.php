    	...
    	if ($cart_contents) {															// checks if cart contents is not empty
      		foreach ($cart_contents as $cart_item) {								// loop through items in cart
				$menu_data = $this->Cart_model->getMenu($cart_item['id']);				// get menu data based on cart item id from getMenu method in Menus model
 				...
 				if (!$menu_data) {
					$data['cart_alert'] = $this->lang->line('alert_menu');
 					$cart_status = FALSE; 				
 				} else if ($menu_data['stock_qty'] <= 0) {												// checks if stock quantity is less than or equal to zero
					$data['cart_alert'] = sprintf($this->lang->line('alert_stock'), $cart_item['name']);
 					$cart_status = FALSE;
 				} else if ($cart_item['qty'] < $menu_data['minimum_qty']) {									// checks if stock quantity is less than or equal to zero
					$data['cart_alert'] = sprintf($this->lang->line('alert_min_qty'), $cart_item['name'], $menu_data['minimum_qty']);
 					$cart_status = FALSE;
				} else {				
 					$cart_status = TRUE;
				}
...
/* Location: ./application/extensions/modules/cart_module/controllers/main/cart_module.php */