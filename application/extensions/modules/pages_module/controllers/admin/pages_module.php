...
class Pages_module extends MX_Controller {
	...
	public function index() {
    	...
    	if (!$this->user->hasPermissions('access', ADMIN_URI.'/pages_module')) {
		...
		$extension = $this->Extensions_model->getExtension('module', 'pages_module');
		
		if (!$this->input->get('id') AND !$this->input->get('name') AND $this->input->get('action') !== 'edit') {
			redirect(ADMIN_URI.'/extensions/edit?name=pages_module&action=edit&id='.$extension['extension_id']);
		}
		...
		if (file_exists(EXTPATH .'modules/pages_module/views/admin/pages_module.php')) { 								//check if file exists in views folder
			$this->load->view('pages_module/admin/pages_module', $data);
		} else {
	...
	public function _updateModule() {
    	if (!$this->user->hasPermissions('modify', ADMIN_URI.'/pages_module')) {
			...
			if ($this->Extensions_model->updateExtension($update, '1')) {
				$this->session->set_flashdata('alert', '<p class="alert-success">Pages Module updated sucessfully.</p>');
			} else {
...
/* Location: ./application/extensions/modules/pages_module/controllers/admin/pages_module.php */