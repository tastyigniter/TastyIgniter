...
class Migration_Schema_a extends CI_Migration {
	...
	public function down() {
		...
		$this->dbforge->drop_column('coupons', array('validity', 'fixed_date', 'fixed_from_time', 'fixed_to_time', 'period_start_date', 'period_end_date', 'recurring_every', 'recurring_from_time', 'recurring_to_time'));
		...
		$this->db->query('ALTER TABLE '.$this->db->dbprefix('menu_options').' CHANGE `option_id` `option_id` INT(11) NOT NULL AUTO_INCREMENT');
		$this->dbforge->drop_column('menu_options', 'menu_option_id');
		...
		$this->dbforge->add_column('order_options', 'option_id');
	...
	public function _menu_option_values() {
		...
		$this->dbforge->add_key('menu_option_value_id', TRUE);
