...
class Coupons_model extends CI_Model {
	...
	public function updateCoupon($update = array()) {
		...
		if (!empty($update['coupon_id'])) {
			...
			$this->Notifications_model->addNotification(array('action' => 'updated', 'object' => 'coupon', 'object_id' => $update['coupon_id'], 'actor_id' => $this->user->getStaffId()));
		}
	...
	public function addCoupon($add = array()) {
		...
		if (!empty($add)) {
			...
			if ($this->db->insert('coupons')) {
				...
				$this->load->model('Notifications_model');
				$this->Notifications_model->addNotification(array('action' => 'added', 'object' => 'coupon', 'object_id' => $query, 'actor_id' => $this->user->getStaffId()));
			}
		}
...
/* Location: ./admin/models/coupons_model.php */