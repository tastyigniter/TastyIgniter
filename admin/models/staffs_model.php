...
class Staffs_model extends CI_Model {
	...
	public function updateStaff($update = array()) {
		...
		if (!empty($update['staff_id'])) {
			...
			$this->Notifications_model->addNotification(array('action' => 'updated', 'object' => 'staff', 'object_id' => $update['staff_id'], 'actor_id' => $this->user->getStaffId()));
		}
	...
	public function addStaff($add = array()) {
		...
		if (!empty($add)) {
			...
			if ($this->db->insert('staffs')) {
				...
				$this->load->model('Notifications_model');
				$this->Notifications_model->addNotification(array('action' => 'added', 'object' => 'staff', 'object_id' => $staff_id, 'actor_id' => $this->user->getStaffId()));
...
/* Location: ./admin/models/staffs_model.php */