...
class Tables_model extends CI_Model {
	...
	public function updateTable($update = array()) {
		...
		if (!empty($update['table_id'])) {
			...
			$this->Notifications_model->addNotification(array('action' => 'updated', 'object' => 'table', 'object_id' => $update['table_id'], 'actor_id' => $this->user->getStaffId()));
		}
	...
	public function addTable($add = array()) {
		...
		if (!empty($add)) {
			if ($this->db->insert('tables')) {
				...
				$this->load->model('Notifications_model');
				$this->Notifications_model->addNotification(array('action' => 'added', 'object' => 'table', 'object_id' => $query, 'actor_id' => $this->user->getStaffId()));
			}
		}
...
/* Location: ./admin/models/tables_model.php */