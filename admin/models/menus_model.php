	...
	public function updateMenu($update = array()) {
		...
		if (!empty($update['menu_id'])) {
			...
			if ($query = $this->db->update('menus')) {
				...
				$this->Notifications_model->addNotification(array('action' => 'updated', 'object' => 'menu', 'object_id' => $update['menu_id'], 'actor_id' => $this->user->getStaffId()));
	...
	public function addMenu($add = array()) {
		...
		if (!empty($add)) {
			if ($this->db->insert('menus')) {
				...
				$this->load->model('Notifications_model');
				$this->Notifications_model->addNotification(array('action' => 'added', 'object' => 'menu', 'object_id' => $menu_id, 'actor_id' => $this->user->getStaffId()));
	...
	public function updateCategory($update = array()) {
		...
		if (!empty($update['category_id'])) {
			...
			if (!empty($update['permalink'])) {
				...
				$this->Permalinks_model->updatePermalink(array('controller' => 'menus', 'permalink' => $update['permalink'], 'query' => 'category_id='.$update['category_id']));
			}
		}
		...
		if (!empty($add)) {
			if ($this->db->insert('categories')) {
				...
				if (!empty($add['permalink'])) {
					$this->load->model('Permalinks_model');
					$this->Permalinks_model->addPermalink(array('controller' => 'menus', 'permalink' => $add['permalink'], 'query' => 'category_id='.$query));
				}
			}
		}
...
/* Location: ./admin/models/menus_model.php */