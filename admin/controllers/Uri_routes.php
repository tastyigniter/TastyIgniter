	...
	protected function validateForm() {
		if ($this->input->post('routes')) {
			...
			foreach ($this->input->post('routes') as $key => $value) {
				$rules[] = array('routes[' . $key . '][uri_route]', 'URI Route', 'xss_clean|trim|required|min_length[2]|max_length[255]');
				$rules[] = array('routes[' . $key . '][controller]', 'Controller', 'xss_clean|trim|required|min_length[2]|max_length[128]');
			}
