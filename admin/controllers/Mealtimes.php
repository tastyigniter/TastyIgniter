	...
	protected function validateForm() {
		if ($this->input->post('mealtimes')) {
			...
			foreach ($this->input->post('mealtimes') as $key => $value) {
				$rules[] = array('mealtimes[' . $key . '][mealtime_id]', 'lang:label_mealtime_id', 'xss_clean|trim|required|integer');
				$rules[] = array('mealtimes[' . $key . '][mealtime_name]', 'lang:label_mealtime_name', 'xss_clean|trim|required|min_length[2]|max_length[128]');
				$rules[] = array('mealtimes[' . $key . '][start_time]', 'lang:label_start_time', 'xss_clean|trim|required|valid_time');
				$rules[] = array('mealtimes[' . $key . '][end_time]', 'lang:label_end_time', 'xss_clean|trim|required|valid_time');
				$rules[] = array('mealtimes[' . $key . '][mealtime_status]', 'lang:label_mealtime_status', 'xss_clean|trim|required|integer');
			}

			return $this->Mealtimes_model->set_rules($rules)->validate();
		}
	}
