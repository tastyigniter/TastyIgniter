...
class Messages extends Admin_Controller
{
	...
	public $sort = ['send_type', 'date_added'];
	...
	public function __construct()
	{
		...
		$this->folders = [
			'inbox'   => ['title' => $this->lang->line('text_inbox'), 'icon' => 'fa-inbox', 'badge' => $message_unread, 'url' => $this->pageUrl()],
			'draft'   => ['title' => $this->lang->line('text_draft'), 'icon' => 'fa-file-text-o', 'badge' => '', 'url' => $this->pageUrl($this->index_url . '/draft')],
			'sent'    => ['title' => $this->lang->line('text_sent'), 'icon' => 'fa-paper-plane-o', 'badge' => '', 'url' => $this->pageUrl($this->index_url . '/sent')],
			'all'     => ['title' => $this->lang->line('text_all'), 'icon' => 'fa-briefcase', 'badge' => '', 'url' => $this->pageUrl($this->index_url . '/all')],
			'archive' => ['title' => $this->lang->line('text_archive'), 'icon' => 'fa-archive', 'badge' => '', 'url' => $this->pageUrl($this->index_url . '/archive')],
		];

		$this->labels = [
			'account' => ['title' => $this->lang->line('text_account'), 'icon' => 'fa-user', 'url' => page_url() . '?filter_type=account'],
			'email'   => ['title' => $this->lang->line('text_email'), 'icon' => 'fa-at', 'url' => page_url() . '?filter_type=email'],
		];
	}
	...
	public function index()
	{
		...
		$this->template->setButton($this->lang->line('button_compose'), ['class' => 'btn btn-primary', 'href' => site_url('messages/compose')]);
	...
	public function all()
	{
		...
		$this->setFilter(['filter_folder' => 'all']);
	...
	public function draft()
	{
		...
		$this->setFilter(['filter_folder' => 'draft']);
	...
	public function sent()
	{
		...
		$this->setFilter(['filter_folder' => 'sent']);
		$data = $this->getList();
	...
	public function archive()
	{
		...
		$this->setFilter(['filter_folder' => 'archive']);
		$data = $this->getList();
	...
	public function view()
	{
		...
		if ($this->input->post('message_state')) {
			...
			$this->Messages_model->updateState($message_info['message_meta_id'], $this->user->getStaffId(), 'read', $message_folder, FALSE);
		}

		$this->template->setTitle($this->lang->line('text_title'));
		$this->template->setHeading(sprintf($this->lang->line('text_edit_heading'), $this->lang->line('text_view')));
		$this->template->setButton($this->lang->line('button_icon_back'), ['class' => 'btn btn-default', 'href' => site_url($previous_uri)]);
	...
	public function compose()
	{
		...
		$this->template->setButton($this->lang->line('button_send'), ['class' => 'btn btn-success', 'onclick' => '$(\'#compose-form\').submit();']);
		$this->template->setButton($this->lang->line('button_save_draft'), ['class' => 'btn btn-default', 'onclick' => 'saveAsDraft();']);
		$this->template->setButton($this->lang->line('button_icon_back'), ['class' => 'btn btn-default', 'href' => site_url('messages')]);
	...
	{
		$this->setFilter(['filter_folder' => 'inbox', 'limit' => '10']);
	...
	public function getList()
	{
		...
		$data['messages'] = [];
		$results = $this->Messages_model->paginateWithFilter($this->getFilter());
		foreach ($results->list as $result) {
			...
			$folder = 'archive';
			if (empty($result['deleted']) AND $result['status'] == '1' AND $result['item'] === 'staff_id') {
				$folder = 'inbox';
			...
			$compose_url = $this->pageUrl($this->edit_url, ['id' => $result['message_id']]);
			$view_url = $this->pageUrl($this->view_url, ['id' => $result['message_id'], 'folder' => $data['filter_folder']]);
			$body = strip_tags(html_entity_decode($result['body'], ENT_QUOTES, 'UTF-8'));
			$data['messages'][] = array_merge($result, [
		...
		$data['message_dates'] = $this->Messages_model->getMessageDates();

		return $data;
	}

	protected function getView($message_info = [])
	{
		$data = $message_info;
		...
		if ($recipients) {
			foreach ($recipients as $recipient) {
				...
				$data['recipients'][] = array_merge($recipient, [
	...
	public function getForm($message_info = [])
	{
		$data = $message_info;
		...
		if (!empty($message_info['message_id'])) {
			$message_id = $message_info['message_id'];
			$data['_action'] = $this->pageUrl($this->edit_url, ['id' => $message_id]);
		}
	...
	{
		$rules[] = ['recipient', 'lang:label_to', 'xss_clean|trim|required|min_length[2]|max_length[128]'];
		$rules[] = ['subject', 'lang:label_subject', 'xss_clean|trim|required|min_length[2]|max_length[128]'];
		$rules[] = ['body', 'lang:label_body', 'required|min_length[3]'];
