...
class Address extends Main_Controller
{
	...
	public function index()
	{
		...
		if ($results->list) {
			foreach ($results->list as $result) {                                                        // loop through the customer address data
				$data['addresses'][] = array_merge($result, [                                                    // create array of customer address data to pass to view
	...
	protected function validateForm()
	{
		...
		$rules[] = ['address[address_1]', 'lang:label_address_1', 'xss_clean|trim|required|min_length[3]|max_length[128]|get_lat_lng[address]'];
		...
		$rules[] = ['address[postcode]', 'lang:label_postcode', 'xss_clean|trim|min_length[2]|max_length[11]'];
