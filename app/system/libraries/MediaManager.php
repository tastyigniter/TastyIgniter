...
class MediaManager
{
    ...
    {
        $this->_sub_folder = ($path !== '') ? explode('/', $path) : [];
    ...
    public function fetchGalleries()
    {
        ...
        foreach ($galleries as $key => $value) {
            $_galleries[] = ['name' => basename($value), 'path' => $value];
        }

        return $_galleries;
    }

    public function fetchFiles($path, $sort = [])
    {
        if (!is_dir($this->_image_path.$this->_root_folder.$path)) return [];
        ...
        foreach (glob($this->_image_path.$this->_root_folder.$path.'*') as $key => $file_path) {
            ...
            if (is_file($file_path) AND !in_array($file_name, $this->_hidden_files)) {
                ...
                $u_files[] = ['name' => $file_name, 'type' => $file_type, 'date' => $date, 'size' => $size, 'ext' => $file_ext, 'perms' => $perms];
            }
        }
    ...
    public function getThumbnail($file_name, $sub_folder = '')
    {
        ...
        return ['dimension' => $img_dimension, 'url' => $thumb_url];
    }
    ...
    public function _folderTree($directory, $return_link, $parent = '')
    {
        ...
        if ($directory_map = File::glob($directory.'*', GLOB_ONLYDIR)) {
