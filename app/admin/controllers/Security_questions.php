    ...
    protected function validateForm()
    {
        if ($this->input->post('questions')) {
            ...
            foreach ($this->input->post('questions') as $key => $value) {
                $rules[] = ['questions['.$key.'][question_id]', 'lang:label_question', 'xss_clean|trim|required|integer'];
                $rules[] = ['questions['.$key.'][text]', 'lang:label_answer', 'xss_clean|trim|required|min_length[2]|max_length[128]'];
            }
