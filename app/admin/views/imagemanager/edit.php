<?php if ($popup !== 'iframe') { ?>
        ...
        <div id="image-manager" style="padding: 3px 0px 0px 0px;">
            <iframe src="<?= admin_url('image_manager?popup=iframe'); ?>"
                    width="100%"
                    height="550"
...
<?php }
    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            var allowed_ext = new Array('<?= implode("','", $allowed_ext); ?>')
            var maxSizeUpload = <?= round($max_size_upload / 1024, 2); ?>
        </script>
    </head>
    <body>
    ...
    <input type="hidden" id="total_files" value="<?= $total_files; ?>"/>
    ...
    <input type="hidden" id="current_folder" value="<?= $current_folder; ?>"/>
    ...
    <div id="folders_list" style="display:none;">
        ...
        <span class="help-block small"><?= lang('help_existing_files'); ?></span>
    </div>
    ...
    <nav class="navbar navbar-default navbar-menu" role="navigation">
        <div class="container-fluid navbar-fixed-top">
            ...
            <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                <div class="btn-toolbar" role="toolbar">
                    <div class="col-xs-12 col-sm-2 wrap-none">
                        <div class="btn-group">
                            <a class="btn btn-default navbar-btn btn-back <?= $back; ?>"
                               title="Back"
                               href="<?= $back_url; ?>"><i class="fa fa-arrow-left"></i></a>
                            <a id="refresh"
                               title="Refresh"
                               class="btn btn-default navbar-btn btn-refresh"
                               href="<?= $refresh_url; ?>"><i class="fa fa-refresh"></i></a>
                        </div>
                    </div>
                    <div class="col-xs-12 col-sm-4 wrap-none">
                        <div class="btn-group">
                            <?php if ($uploads) { ?>
                                <button type="button"
                                        class="btn btn-default navbar-btn btn-upload"><i class="fa fa-upload"></i>&nbsp;&nbsp;
                                    <small><?= lang('button_upload'); ?></small>
                                </button>
                            <?php } ?>
                        </div>
                    </div>
                    <div class="col-xs-2 col-sm-1 wrap-none">
                        <div class="btn-group">
                            <a class="btn btn-default navbar-btn btn-options"
                               title="<?= lang('button_option'); ?>"
                               href="<?= admin_url('settings#image-manager'); ?>"
                               target="_parent"><i class="fa fa-gear"></i></a>
                        </div>
                    </div>
                    <div class="col-xs-2 col-sm-1 wrap-none">
                        <div class="btn-group">
                            <div class="dropdown">
                                <a class="btn btn-default navbar-btn btn-sort dropdown-toggle"
                                ...
                                <ul class="dropdown-menu dropdown-sorter" role="menu">
                                    ...
                                    <li>
                                        <a class="sorter"
                                           data-sort="name"><?= ($sort_by === 'name') ? $sort_icon : ''; ?><?= lang('label_name'); ?></a>
                                    </li>
                                    <li>
                                        <a class="sorter"
                                           data-sort="date"><?= ($sort_by === 'date') ? $sort_icon : ''; ?><?= lang('label_date'); ?></a>
                                    </li>
                                    <li>
                                        <a class="sorter"
                                           data-sort="size"><?= ($sort_by === 'size') ? $sort_icon : ''; ?><?= lang('label_size'); ?></a>
                                    </li>
                                    <li>
                                        <a class="sorter"
                                           data-sort="extension"><?= ($sort_by === 'extension') ? $sort_icon : ''; ?><?= lang('label_type'); ?></a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <div class="btn-group btn-block">
                            <div class="navbar-form input-group">
                                <span id="btn-clear"
                                      ...
                                      title="<?= lang('text_clear'); ?>"><i id="filter-clear"
                                ...
                                <input type="text"
                                       ...
                                       class="form-control"
                                       value="<?= $filter; ?>"
                                       placeholder="<?= lang('text_filter_search'); ?>"/>
                                <span id="btn-search" class="input-group-addon" title="<?= lang('text_search'); ?>"><i
            ...
            <ol class="breadcrumb">
                ...
                <li>
                    <?php if ($new_folder) { ?>
                        <a class="btn btn-new-folder"
                           title="<?= lang('text_new_folder'); ?>"
                           href="#"><i class="fa fa-folder"></i></a>&nbsp;&nbsp;&nbsp;
                    <?php } ?>
                    <?php if ($rename) { ?>
                        <a class="btn btn-rename"
                           title="<?= lang('text_rename_folder'); ?>"
                           data-name="<?= $current_folder; ?>"
                           data-path="<?= $parent_folder; ?>"
                           href="#"><i class="fa fa-pencil"></i></a>&nbsp;&nbsp;&nbsp;
                    <?php } ?>
                    <?php if ($delete) { ?>
                        <a class="btn btn-delete"
                           title="<?= lang('text_delete_folder'); ?>"
                           data-name="<?= $current_folder; ?>"
                           data-path="<?= $parent_folder; ?>"
                ...
                <div class="uploader-box">
                    <div class="tabbable upload-tabbable"> <!-- Only required for left/right tabs -->
                        <form role="form"
                              ...
                              class="dropzone">
                            <input type="hidden" name="sub_folder" value="<?= $sub_folder; ?>"/>
                            <div class="fallback">
                                <?= lang('button_upload'); ?>:<br/>
                    ...
                    <div id="selectable" class="col-xs-9 wrap-none">
                        ...
                        <div class="media-list row">
                            <?php foreach ($files as $file) { ?>
                                <div class="thumbnail-each col-xs-3 col-sm-2 <?= $file['html_class']; ?>">
                                    <figure class="thumbnail"
                                            data-type="<?= $file['type']; ?>"
                                            data-name="<?= $file['name']; ?>"
                                            data-path="<?= $file['path']; ?>">
                                        <a class="link" title="<?= $file['size']; ?>">
                                            ...
                                            <figcaption class="caption">
                                                <h4 class="ellipsis">
                                                    <span class=""><?= $file['human_name']; ?></span>
                                                </h4>
                                            </figcaption>
                                        </a>
                                        <div class="info">
                                            ...
                                            <ul class="get_info">
                                                ...
                                                <li class="file-permission">
                                                    ...
                                                    <?php if ($file['perms'] === '04' OR $file['perms'] === '05') { ?>
                                                        <?= lang('text_read_only'); ?>
                                                    <?php }
                                                    else if ($file['perms'] === '06' OR $file['perms'] === '07') { ?>
    ...
    <nav class="navbar navbar-default navbar-statusbar navbar-fixed-bottom" role="navigation">
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-sm-4">
                    <p class="navbar-text">
                        <span class="total-selected">0</span><span class="total-selected-text"> of </span><?= sprintf(lang('text_footer_note'), $total_files, $folder_size); ?>
                    </p>
                </div>
                <div class="col-sm-4 text-right">
                    <?php if ($field_id) { ?>
                        <a class="btn btn-primary btn-choose disabled"><?= lang('text_choose'); ?></a>
                    <?php } ?>
                </div>
            </div>
        </div>
