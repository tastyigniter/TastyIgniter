 ...
 *
 * @category       Libraries
 * @package        Igniter\Libraries\Hub_manager.php
...
class Hub_manager
{
	...
	public function __construct()
	{
		...
		$this->CI->load->driver('cache', ['adapter' => $this->CI->config->item('cache_driver')]);
	...
	public function listItems($filter = [])
	{
		...
		if (!$items = $this->CI->cache->get($cacheFile)) {
			$items = $this->requestRemoteData('items', $filter);

			if (!empty($items) AND is_array($items))
				$this->CI->cache->save($cacheFile, $items, $this->cacheLife);
		}
	...
	public function requestUpdateList($itemNames, $force = FALSE)
	{
		...
		if ($force OR !$response = $this->CI->cache->get($cacheFile)) {
			$response = $this->requestRemoteData('core/check', ['items' => $itemNames]);

			if (is_array($response)) {
				$response['check_time'] = time();
				$this->CI->cache->save($cacheFile, $response, $this->cacheLife);
			}
		}
