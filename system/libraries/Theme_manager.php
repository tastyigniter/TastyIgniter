 ...
 *
 * @category       Libraries
 * @package        Igniter\Libraries\Theme_manager.php
...
class Theme_manager
{
	...
	public function loadTheme($themeCode, $path)
	{
        ...
        $themeObject->partialAreas = $this->findPartialAreas($themeCode);
    ...
    {
        return $this->CI->config->item(APPDIR, 'default_themes');
    }
...
//			}
	...
	{
        $folders = [];
        foreach ($this->config['view_folders'] as $view_folder) {
            $domain = (basename($view_folder) == 'themes') ? MAINDIR : ADMINDIR;
            $folders[$domain] = $view_folder;
		}
