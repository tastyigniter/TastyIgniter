...
class Location
{
    ...
    {
        if (empty($this->locationsModel)) {
            $this->CI->load->model('Locations_model');
            $this->locationsModel = $this->CI->Locations_model->isEnabled()->get();
        }

        $locations = [];
        if (!$this->locationsModel->isEmpty()) {
            foreach ($this->locationsModel->toArray() as $result) {
