 ...
 *
 * @category       Libraries
 * @package        Igniter\Core\TI_Lang.php
...
//
		...
		if ($path === FALSE) {
			...
			if ($lang = Modules::load_file($_langfile, $path, 'lang')) {
				$_module = dirname($langfile);
				if ($return) return array($_module => $lang);
				$this->language = array_merge($this->language, array($_module => $lang));
	...
	public function line($line, $params = NULL, $log_errors = TRUE) {
		if (sscanf($line, 'lang:%s', $__line) === 1) {
			return $this->line($__line, NULL, FALSE);
		}

		$_line = explode('.', $line);
		if (count($_line) > 1 AND !isset($this->language[$_line[0]])) {
			$this->load($_line[0] . '/' . $_line[0], '', FALSE, TRUE, '', $_line[0]);
		}
		...
		if ($value AND !empty($params) AND is_array($params)) {
		...
		if (!($value = array_get($this->language, $line, FALSE))) {
			if (isset($this->language[$line])) {
				...
				foreach ($this->language as $lang)
					if (isset($lang[$line])) $value = $lang[$line];
			}
		}
