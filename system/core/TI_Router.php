 ...
 *
 * @category       Libraries
 * @package        Igniter\Core\TI_Router.php
...
//    private $routes =	array();
    ...
    /**
     * Overrides parent to replace APPPATH
     */
    protected function _set_default_controller()
    {
        ...
        if ( ! file_exists(TI_APPPATH.'controllers/'.$this->directory.ucfirst($class).'.php'))
        {
	...
	public function locate($segments) {
		...
		if ($directory) {
			if (is_file(TI_APPPATH . 'controllers/' . $module . '/' . ucfirst($directory) . $ext)) {
			...
			if ($controller) {
				if (is_file(TI_APPPATH . 'controllers/' . $module . '/' . $directory . '/' . ucfirst($controller) . $ext)) {
		...
		if (is_dir(TI_APPPATH . 'controllers/' . $module . '/')) {
			$this->directory = $module . '/';
		...
		if (is_file(TI_APPPATH . 'controllers/' . ucfirst($module) . $ext)) {
			return $segments;
		}
