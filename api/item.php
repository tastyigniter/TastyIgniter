...
if($lang=='fr'){
$sqlquery=mysqli_query($conn,"SELECT menu_id,menu_french_name,menu_french_description,menu_price,stock_qty,menu_photo FROM  `ti_menus` WHERE  `menu_category_id`='$cat_id' AND `menu_photo` IS NOT NULL");
while($row=mysqli_fetch_array($sqlquery))
	{
		$category_data['item_id']=$row['menu_id'];
		$category_data['name']=$row['menu_french_name'];
		$category_data['description']=$row['menu_french_description'];
