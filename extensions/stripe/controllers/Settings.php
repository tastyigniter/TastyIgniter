...
class Settings extends Admin_Controller {
	...
	public function index($module = array()) {
		...
		if ($this->input->post() AND $this->_updateStripe() === TRUE) {
			...
			$this->redirect(admin_extension_url('stripe/settings'));
		}

		return $this->load->view('stripe/settings', $data, TRUE);
	}

	protected function _updateStripe() {
		if ($this->input->post() AND $this->validateForm() === TRUE) {

			if ($this->Extensions_model->updateSettings('stripe', $this->input->post())) {
...
/* Location: ./extensions/stripe/controllers/Settings.php */