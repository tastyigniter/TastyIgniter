...
class Admin_options extends Extension {
	...
	public function options($options = array()) {
    	...
    	if (!$this->user->hasPermissions('access', 'categories_module')) {
  			redirect('permission');
		}
	...
	public function _updateModule() {
    	if (!$this->user->hasPermissions('modify', 'categories_module')) {
			...
			if ($this->Extensions_model->updateExtension($update, '1')) {
				$this->alert->set('success', 'Categories Module updated sucessfully.');
			} else {
