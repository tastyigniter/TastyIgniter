...
class Settings extends Admin_Controller {
	...
	public function index($module = array()) {
		...
		if ($this->input->post() AND $this->_updateModule() === TRUE) {
			...
			$this->redirect(admin_extension_url('slideshow/settings'));
		}

        return $this->load->view('slideshow/settings', $data, TRUE);
	}

	protected function _updateModule() {
    	if ($this->validateForm() === TRUE) {

		    if ($this->Extensions_model->updateSettings('slideshow', $this->input->post())) {
...
/* Location: ./extensions/slideshow/controllers/Settings.php */