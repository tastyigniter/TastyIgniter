...
class Settings extends Admin_Controller {
	...
	public function index($module = array()) {
		...
		if ($this->input->post() AND $this->_updateAuthorizeNetAIM() === TRUE) {
			...
			$this->redirect(admin_extension_url('authorize_net_aim/settings'));
		}

		return $this->load->view('authorize_net_aim/settings', $data, TRUE);
	}
	...
	protected function _updateAuthorizeNetAIM() {
		...
		if ($this->input->post() AND $this->validateForm() === TRUE) {
			...
			if ($this->Extensions_model->updateSettings('authorize_net_aim', $this->input->post())) {
...
/* Location: ./extensions/authorize_net_aim/controllers/Settings.php */