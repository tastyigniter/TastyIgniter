<div class="container-fluid py-3">
    <div class="d-flex flex-wrap align-items-start justify-content-between gap-3 mb-4">
        ...
        <div class="d-flex flex-wrap gap-2 align-items-center">
            <span class="badge bg-{{ $staffActive ? 'success' : 'danger' }}">{{ $operationalProfileLabel }}</span>
    ...
    @if($workspace === 'overview')
        <div class="row g-3 mb-4">
            ...
            <div class="col-sm-6 col-xl-3"><div class="card h-100"><div class="card-body">
                ...
                <h2 class="h5 mb-2">{{ $staffName }}</h2>
                <p class="mb-2">{{ $operationalProfileLabel }}</p>
                <span class="badge bg-{{ $staffActive ? 'success' : 'danger' }}">{{ $staffActive ? 'Active' : 'Inactive' }}</span>
            </div></div></div>
            <div class="col-sm-6 col-xl-3"><div class="card h-100"><div class="card-body">
                <div class="text-muted small text-uppercase mb-2">Assigned Branches</div>
                <h2 class="display-6 mb-2">{{ $assignedLocationCount }}</h2>
                <p class="text-muted mb-0">{{ $assignedLocations->pluck('location_name')->join(', ') ?: 'No active branches assigned' }}</p>
            </div></div></div>
            <div class="col-sm-6 col-xl-3"><div class="card h-100"><div class="card-body">
                <div class="text-muted small text-uppercase mb-2">Operational Access</div>
                <h2 class="display-6 mb-2">{{ $accessibleModuleCount }}</h2>
                <p class="text-muted mb-0">{{ $accessSummary->join(', ') ?: 'Operations overview only' }}</p>
            </div></div></div>
        </div>
        ...
        <div class="row g-3 mb-4">
            <section class="col-lg-8">
                <div class="card h-100">
                    ...
                    <div class="card-body">
                        <div class="row g-2">
                            @forelse($quickActions as $module)
            ...
            <section class="col-lg-4">
                <div class="card h-100">
                    ...
                    <ul class="list-group list-group-flush">
                        <li class="list-group-item d-flex justify-content-between"><span>Profile</span><strong>{{ $operationalProfileLabel }}</strong></li>
    ...
    @foreach($readinessMessages as $readinessMessage)
        <div class="alert alert-warning" role="alert"><i class="fa fa-exclamation-triangle me-2"></i>{{ $readinessMessage }}</div>
    @endforeach
