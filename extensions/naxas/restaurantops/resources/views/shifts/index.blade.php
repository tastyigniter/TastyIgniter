<div class="container-fluid py-3">
    ...
    @forelse($records as $shift)<tr><td><a href="{{ route('naxas.restaurantops.shifts.show', $shift) }}">{{ $shift->id }}</a></td><td>{{ $shift->staff_id }}</td><td>{{ $shift->opened_at }}</td><td><span class="badge bg-secondary">{{ str($shift->status->value)->replace('_', ' ') }}</span></td><td>{{ $shift->expected_cash === null ? '—' : currency_format($shift->expected_cash) }}</td><td>{{ $shift->variance === null ? '—' : currency_format($shift->variance) }}</td></tr>@empty<tr><td colspan="6" class="text-center text-muted py-5">@lang('Naxas.RestaurantOps::default.shifts.empty')</td></tr>@endforelse
