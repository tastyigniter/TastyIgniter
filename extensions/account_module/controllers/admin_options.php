...
class Admin_options extends Extension {
	...
	public function options($options = array()) {
    	...
    	if (!$this->user->hasPermissions('access', 'account_module')) {
  			redirect('permission');
		}
	...
	public function _updateModule() {
    	if (!$this->user->hasPermissions('modify', 'account_module')) {
			...
			if ($this->Extensions_model->updateExtension($update, '1')) {
				$this->alert->set('success', 'Account Module updated sucessfully.');
			} else {
